...
function formatBeatDuration(bpm) {
  return `${(60 / bpm).toFixed(3)}s`;
}

function RunnerCadenceSvg({ mode }) {
  const isHigh = mode === "high";
  const bpm = isHigh ? 180 : 90;
  const beatDuration = formatBeatDuration(bpm);
  ...
  return (
    ...
    >
      ...
      {isHigh ? (
        <g className="cadence-diagram is-high">
          ...
          <g className="high-strides" aria-hidden="true">
            {[110, 164, 218, 272, 326, 380, 434, 488, 542, 596].map((x, index) => (
              <line
                key={x}
                x1={x}
                x2={x + 24}
                y1="408"
                y2="408"
                style={{ "--stride-delay": `${(index * beatSeconds * 0.5).toFixed(3)}s` }}
              />
            ))}
          </g>
          ...
          <g className="low-strides" aria-hidden="true">
            {[120, 288, 500].map((x, index) => (
              <line
                ...
                y1="408"
                y2="408"
                style={{ "--stride-delay": `${(index * beatSeconds).toFixed(3)}s` }}
              />
            ))}
          </g>
      ...
      <g className="runner-position" aria-hidden="true" transform="translate(238 118) scale(2.08)">
        <g className={`real-runner ${isHigh ? "is-high" : "is-low"}`}>
...
function HighBpmPage() {
  ...
  const diagramBpm = diagramMode === "high" ? 180 : 90;
    ...
    <main className="bpm-lesson-shell">
      <section className="bpm-lesson-hero" aria-labelledby="bpm-lesson-title">
        ...
        <div className="lesson-visual">
          ...
          <p className="diagram-tempo-readout" aria-live="polite">
            Animation paced at {diagramBpm} BPM
          </p>
